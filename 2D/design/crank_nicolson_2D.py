...
class groundState2DCN:
    ...
    def __init__(self,xMin,xMax, yMin,yMax, dtau, Jx, Jy, D, pot, a=-1j, hbar=1, m=1):
    ...
    def step(self):
        ...
        #Propagation in x
        C = self.Bx.dot(self.U.T)+0.5*self.dt*F.T
        C[0,:] = 0
        C[-1,:] = 0
        self.U = lin.solve_banded((1,1), self.Ax, C, check_finite=False).T
        
        F = 3/2*(self.pot.f)(self.X, self.Y, self.U)-1/2*(self.pot.f)(self.X, self.Y, self.oldU)
        #Propagation in y
        C = (self.By.dot(self.U))+0.5*self.dt*F
        C[0,:] = 0
        C[-1,:] = 0
        self.U = lin.solve_banded((1,1), self.Ay, C, check_finite=False)
    
    def change_a(self, a):
        """Change the value of a.
            Useful to see the real time propagation after the imaginary time propagation
        """
        self.dt = a*self.dtau
